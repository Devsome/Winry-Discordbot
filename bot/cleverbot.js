...
exports.cleverbot = function(bot, msg) {
	...
	if (text) {
		...
		if (!msg.channel.isPrivate)
			console.log(cGreen(msg.channel.server.name) + " > " + cGreen(msg.author.username) + " > " + cRed("@" + bot.user.username) + " " + msg.content.replace(new RegExp(`^<@!?${bot.user.id}>`), "").replace(" ", "").replace(/\n/g, " "));
		else
			console.log(cGreen(msg.author.username) + " > " + cRed("@" + bot.user.username) + " " + msg.content.replace(new RegExp(`^<@!?${bot.user.id}>`), "").replace(/\n/g, " "));
		...
		if (!msg.channel.isPrivate)
			console.log(cRed(msg.channel.server.name) + " > " + cGreen(msg.author.username) + " > " + cRed("@" + bot.user.username) + " " + msg.content.replace(new RegExp(`^<@!?${bot.user.id}>`), "").replace(" ", "").replace(/\n/g, " "));
		else
			console.log(cGreen(msg.author.username) + " > " + cRed("@" + bot.user.username) + " " + msg.content.replace(new RegExp(`^<@!?${bot.user.id}>`), "").replace(/\n/g, " "));
